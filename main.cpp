...
public:
    ...
    void resize(long int size) { //? Resizing the node array to a given size and copying its elements;
        ...
        for (int i = 0; i < (last_item_index +
                             1); ++i) { // The last item index is added by one because it's "pointing" to the last element in the array, the reason is because if not added by one 'i' would go only to the pre-last element and would miss an element in the array;
...
public:
    ...
    void hashtable_add(string kmer, long int current_position, unsigned long int key) {//! Adds a kmer to the table;
        ...
        for (int i = 0; i < table_sz; ++i) { //! If the item is already inserted in the hashtable;
            long int index = (key + i) % table_sz;
            if (Hash_array[index].correspondent_kmer() == kmer) {
                Hash_array[index].push_back(current_position);
                return;
            }
        }
    ...
    long int in_table(string kmer,long int key) {
        for (int i = 0; i < table_sz; ++i) {
            long int ind = (key + i) % table_sz;
            if (Hash_array[ind].correspondent_kmer() == kmer) {
                return ind;
            }
        }
        return (-2);
...
int main() {
    ...
    for (int i = 0; i < (text.size() -
                         kmer_size); ++i) {//? MAIN loop of the hashtable; Which creates the hashtable and add the items in the text;
        string temp_string;
    ...
    for (int i = 0; i < consultas; ++i) {
        ...
        unsigned long int key_2 = key_translator(kamer,kmer_size);
        index = My_table->in_table(kamer,key_2);
